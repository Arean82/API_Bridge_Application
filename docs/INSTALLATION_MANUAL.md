# Synora Bridge – Simple Installation Manual
...
You now have a working development copy of the Synora Bridge ready for the next step.
...
If you want to run multiple Synora Bridge instances across several servers for load balancing, you will need to configure a distributed task broker. See the **[Distributed Execution & Scaling Guide](SCALING_REDIS_MEMURAI.md)** for instructions on setting up Redis (Linux) or Memurai (Windows).
...
```ini
[Unit]
Description=Synora Bridge (PyInstaller binary)
After=network.target
