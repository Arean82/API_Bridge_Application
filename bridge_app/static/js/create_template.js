class CreateTemplateController {
        ...
        if (this.cloneData) {
            ...
            if (cloneData.sources && cloneData.sources.length > 0) {
                this.sources = cloneData.sources.map(s => ({
                    ...
                    source_type: s.source_type || 'rest',
                ...
                this.sources = [{ id: Date.now(), source_type: 'rest', graphql_query: '', connectionId: '', selectedApi: '', url: cloneData.partner_url, auth_token: cloneData.partner_auth_token || '', availableApis: [] }];
            }

        }

        if (!this.sources || this.sources.length === 0) {
            this.sources = [{ id: Date.now(), source_type: 'rest', graphql_query: '', connectionId: '', selectedApi: '', url: '', auth_token: '', availableApis: [] }];
        }
    ...
    addSource() {
        this.sources.push({ id: Date.now(), source_type: 'rest', graphql_query: '', connectionId: '', selectedApi: '', url: '', auth_token: '', availableApis: [] });
    ...
    get currentApiFields() {
        ...
        this.sources.forEach((src, idx) => {
            if (src.source_type === 'graphql') {
                if (src.graphql_query) {
                    let match = src.graphql_query.match(/query\s*\{([^}]+)\}/);
                    if (match && match[1]) {
                        let fields = match[1].split(/\s+/).filter(f => f.trim().length > 0);
                        fields.forEach(f => {
    ...
    renderSources() {
        ...
        this.sources.forEach((src, idx) => {
            ...
            let isGraphql = src.source_type === 'graphql';
            let filteredConns = this.swaggerConnections.filter(c => {
                let cType = c.connection_type || 'rest';
                return isGraphql ? cType === 'graphql' : cType === 'rest';
            });
            let html = `
                ...
                <div class="flex justify-between items-center mb-4">
                    <h4 class="font-bold text-sm theme-text-muted">ENDPOINT <span>${idx + 1}</span></h4>
                    <select class="theme-input p-1 text-xs border border-gray-300 rounded focus:ring-indigo-500 src-type-sel" data-idx="${idx}">
                        <option value="rest" ${!isGraphql ? 'selected' : ''}>REST Source</option>
                        <option value="graphql" ${isGraphql ? 'selected' : ''}>GraphQL Source</option>
                    </select>
                </div>
                <div class="space-y-4">
                    <div>
                        <div class="flex justify-between items-center mb-1">
                            <label class="block text-sm font-medium">${isGraphql ? 'GraphQL Connection' : 'REST API Connection'}</label>
                            <button class="text-indigo-600 hover:text-indigo-800 text-xs font-semibold new-conn-btn" type="button">+ New Connection</button>
                        </div>
                        <select class="theme-input w-full p-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 src-conn-sel" data-idx="${idx}">
                            ...
                            ${filteredConns.map(c => `<option value="${c.id}" ${src.connectionId == c.id ? 'selected' : ''}>${c.name}</option>`).join('')}
                        </select>
                    </div>
                    <div class="col-span-2" style="display: ${isGraphql ? 'none' : 'block'}">
                    ...
                    <div style="display: ${isGraphql ? 'block' : 'none'}" class="border border-indigo-200 dark:border-indigo-900 rounded p-4 bg-indigo-50/30 dark:bg-indigo-900/10">
                        <div class="flex justify-between items-center mb-2">
                            <label class="block text-sm font-medium text-indigo-700 dark:text-indigo-300">GraphQL Query Builder</label>
                            <button type="button" class="text-xs bg-indigo-600 text-white px-2 py-1 rounded hover:bg-indigo-700 introspect-btn" data-idx="${idx}">Fetch Schema</button>
                        </div>
                        <div class="flex gap-4">
                            <div class="w-1/2 bg-white dark:bg-black/20 border border-gray-200 dark:border-gray-700 rounded overflow-y-auto h-48 p-2 text-xs schema-tree-container" data-idx="${idx}">
                                <div class="text-gray-400 italic">Click "Fetch Schema" to introspect endpoint.</div>
                            </div>
                            <div class="w-1/2">
                                <textarea class="theme-input w-full p-2.5 text-sm font-mono src-gql-in" data-idx="${idx}" rows="8" placeholder="query { ... }">${src.graphql_query || ''}</textarea>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            let newConnBtn = div.querySelector('.new-conn-btn');
            if (newConnBtn) {
            ...
            div.querySelector('.src-type-sel').addEventListener('change', (e) => { 
                this.sources[idx].source_type = e.target.value; 
                this.renderSources(); 
            });
            let connSel = div.querySelector('.src-conn-sel');
            if (connSel) connSel.addEventListener('change', (e) => { 
                this.sources[idx].connectionId = e.target.value; 
                let conn = this.swaggerConnections.find(c => c.id == this.sources[idx].connectionId);
                if (conn) {
                    this.sources[idx].url = conn.url || '';
                    this.sources[idx].auth_token = conn.auth_token || '';
                } else {
                    this.sources[idx].url = '';
                    this.sources[idx].auth_token = '';
                }
                if (this.sources[idx].source_type === 'rest') {
                    this.fetchApiDocs(idx); 
                } else {
                    this.renderSources();
                }
            });
            let apiSel = div.querySelector('.src-api-sel');
            if (apiSel) apiSel.addEventListener('change', (e) => { 
            ...
            let gqlIn = div.querySelector('.src-gql-in');
            if (gqlIn) gqlIn.addEventListener('input', (e) => { 
                this.sources[idx].graphql_query = e.target.value; 
                this.updateDestinationFields();
            });
            
            let introBtn = div.querySelector('.introspect-btn');
            if (introBtn) {
                introBtn.addEventListener('click', async (e) => {
                    e.preventDefault();
                    let url = this.sources[idx].url;
                    if (!url) { alert('Please enter a GraphQL Source URL first.'); return; }
                    introBtn.textContent = 'Fetching...';
                    try {
                        const res = await fetch('/api/bridge/graphql_introspect', {
                            method: 'POST',
                            headers: { 'Content-Type': 'application/json' },
                            body: JSON.stringify({ url: url, auth_token: this.sources[idx].auth_token })
                        });
                        if (res.ok) {
                            const schema = await res.json();
                            this.renderSchemaTree(schema, idx);
                        } else {
                            alert('Failed to introspect schema');
                        }
                    } catch (err) {
                        alert('Error: ' + err.message);
                    }
                    introBtn.textContent = 'Fetch Schema';
                });
            }
    ...
    renderSchemaTree(schemaData, idx) {
        if(!schemaData || !schemaData.data || !schemaData.data.__schema) return;
        const schema = schemaData.data.__schema;
        const queryType = schema.queryType ? schema.types.find(t => t.name === schema.queryType.name) : null;
        if(!queryType) return;
        
        let container = document.querySelector(`.schema-tree-container[data-idx="${idx}"]`);
        if(!container) return;
        
        container.innerHTML = '<div class="font-bold mb-1">Query Fields</div><div class="pl-2 space-y-1 tree-root"></div>';
        let root = container.querySelector('.tree-root');
        
        // Simple recursive tree builder for one level deep
        queryType.fields.forEach(field => {
            let fieldDiv = document.createElement('div');
            fieldDiv.className = 'flex items-center gap-2';
            fieldDiv.innerHTML = `<input type="checkbox" class="gql-field-cb" value="${field.name}"> <span>${field.name}</span>`;
            root.appendChild(fieldDiv);
        });
        
        // Listeners
        let checkboxes = container.querySelectorAll('.gql-field-cb');
        checkboxes.forEach(cb => {
            cb.addEventListener('change', () => {
                let selected = Array.from(checkboxes).filter(c => c.checked).map(c => c.value);
                if (selected.length > 0) {
                    this.sources[idx].graphql_query = `query {
  ${selected.join('\\n  ')}
}`;
                } else {
                    this.sources[idx].graphql_query = '';
                }
                let gqlIn = document.querySelector(`.src-gql-in[data-idx="${idx}"]`);
                if(gqlIn) gqlIn.value = this.sources[idx].graphql_query;
                this.updateDestinationFields();
            });
        });
    }
